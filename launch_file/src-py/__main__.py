...
def main():
    ...
    parser.add_argument("raw", type=pathlib.Path, nargs='?', default=None)
    parser.add_argument("-o", "--out", type=pathlib.Path)
    parser.add_argument("-f", "--format", type=pathlib.Path)
    parser.add_argument("-d", "--data", type=format_validator)
    parser.add_argument("-S", action="store_true", dest="only_schema")
    args = parser.parse_args()
    if args.only_schema:
        if args.raw is not None or args.data is not None or args.format is None:
            parser.print_usage()
            exit(1)
        if args.out is None:
            args.out = args.format.with_suffix(".json")
    else:
        if args.raw is None:
            parser.print_usage()
            exit(1)
    ...
    if args.only_schema:
        with open(args.out, "w") as out_file:
            print(f"Writing schema to '{args.out}'")

            schema = {"<checksum>": expected_checksum}
            for num, (name, ty) in mapping.items():
                schema[name] = [num, ty.get_schema()]
